...
static Result<void> invoke_add_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
static Result<void> invoke_sub_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
static Result<void> invoke_mul_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
static Result<void> invoke_div_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
static Result<void> invoke_mod_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
static Result<void> invoke_eq_float64(Environment* env) {
  auto stack = env->call_stack();
  auto frame = TRY(stack->CurrentFrame());
  auto a = TRY(stack->PopPrimitive(frame));
  auto b = TRY(stack->PopPrimitive(frame));
...
Result<void> Executor::InvokeOp(Environment* env) { // NOLINT(readability-convert-member-functions-to-static)
  ...
  switch (op) {
    ...
    case OpCode::NEG_FLOAT64:
      DEBUG_LOG(pc << ": neg_float64" << std::endl);
      return invoke_neg_float64(env);
    case OpCode::ADD_FLOAT64:
      DEBUG_LOG(pc << ": add_float64" << std::endl);
      return invoke_add_float64(env);
    case OpCode::SUB_FLOAT64:
      DEBUG_LOG(pc << ": sub_float64" << std::endl);
      return invoke_sub_float64(env);
    case OpCode::MUL_FLOAT64:
      DEBUG_LOG(pc << ": mul_float64" << std::endl);
      return invoke_mul_float64(env);
    case OpCode::DIV_FLOAT64:
      DEBUG_LOG(pc << ": div_float64" << std::endl);
      return invoke_div_float64(env);
    case OpCode::MOD_FLOAT64:
      DEBUG_LOG(pc << ": mod_float64" << std::endl);
      return invoke_mod_float64(env);
    case OpCode::EQ_FLOAT64:
      DEBUG_LOG(pc << ": eq_float64" << std::endl);
      return invoke_eq_float64(env);
    case OpCode::NE_FLOAT64:
      DEBUG_LOG(pc << ": ne_float64" << std::endl);
      return invoke_ne_float64(env);
    case OpCode::GT_FLOAT64:
      DEBUG_LOG(pc << ": gt_float64" << std::endl);
      return invoke_gt_float64(env);
    case OpCode::GE_FLOAT64:
      DEBUG_LOG(pc << ": ge_float64" << std::endl);
      return invoke_ge_float64(env);
    case OpCode::LT_FLOAT64:
      DEBUG_LOG(pc << ": lt_float64" << std::endl);
      return invoke_lt_float64(env);
    case OpCode::LE_FLOAT64:
      DEBUG_LOG(pc << ": le_float64" << std::endl);
      return invoke_le_float64(env);
    case OpCode::CONV_INT64_FLOAT64:
      DEBUG_LOG(pc << ": conv_int64_float64" << std::endl);
      return invoke_conv_int64_float64(env);
    case OpCode::CONV_FLOAT64_INT64:
      DEBUG_LOG(pc << ": conv_float64_int64" << std::endl);
      return invoke_conv_float64_int64(env);
    default:
      return make_error("Unknown op: " + to_string(static_cast<int>(op)));
  }
}
Result<Value> Executor::CallProcedure(Environment* env,
                                    const std::string& proc_name,
                                    const std::vector<Value>& params) {
  std::string m, p;
  std::tie(m, p) = TRY(parse_procedure(proc_name));
  auto module = TRY(env->module_table()->GetModule(m));
